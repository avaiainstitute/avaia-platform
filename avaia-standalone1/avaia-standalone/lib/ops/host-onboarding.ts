...
// exclusively from conversations.stage/status/created_at, referrals'
// from_stage/to_stage, and profiles/entitlements -- status and progress
// metadata only. Nothing here ever selects messages.content or any other
// conversation text; the query below cannot see what a Host actually said,
// only that a stage exists and whether it's complete. That satisfies the
// build instruction directly: "primarily use status/progress metadata, not
// read someone's private story merely to decide whether to send a
// reminder."
...
/** One pass over every conversation row (small, metadata-only columns) to
 *  classify each Host into exactly one current state, and separately flag
 *  anyone whose most recent conversation has sat 'active' past the stall
 *  threshold. A Host can only be in one current state at a time -- the
 *  furthest stage they've reached -- but stalled-ness is evaluated
 *  independently of that state. */
export async function getHostOnboardingSnapshot(): Promise<{
  ...
  for (const profile of profileRows ?? []) {
    ...
    // Stalled: the Host's single most recent conversation row is still
    // 'active' and old enough to count as paused, regardless of which
    // stage it's at.
...
/** Sends at most one reminder per stalled Host per run, and never a second
 *  reminder of the same type within REMINDER_COOLDOWN_DAYS -- "do not
 *  pressure people" enforced as a real cooldown, not just a tone
 *  instruction. Returns what it actually sent, for the Founder digest and
 *  for a human reviewing cron logs. */
