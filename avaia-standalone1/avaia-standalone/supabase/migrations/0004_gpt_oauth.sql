-- GPT OAuth provider — replaces the copy-paste code handoff. Run once
-- against the live database (Supabase SQL editor), after 0001-0003.
--
-- AVAIA acts as a minimal OAuth 2.0 authorization-code provider for exactly
-- one client: the IAP custom GPT's Action. The Host authenticates once,
-- through a real consent screen, instead of the model needing to remember
-- and reproduce an opaque value across a conversation. Two tables:
--
...
-- referral endpoint resolve identity without any value the model has to
-- carry itself.
--
-- Both locked out of anon/authenticated entirely (RLS enabled, zero
-- policies) — service-role only, same posture as gpt_handoff_sessions and
-- the Stripe webhook.
