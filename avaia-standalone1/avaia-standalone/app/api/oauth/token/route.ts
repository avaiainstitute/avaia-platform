...
// PROOF OF CONCEPT — the token-exchange half of AVAIA's minimal OAuth
// provider. Called server-to-server by OpenAI's infrastructure (not the
// Host's browser) once the Host has approved /oauth/authorize. Trades a
// short-lived authorization code for a long-lived bearer access token,
// bound to the Host who approved it.
//
// Client credentials may arrive either as HTTP Basic auth (client_secret_basic)
// or in the request body (client_secret_post) — supporting both since the
// exact method GPT Builder's OAuth implementation uses isn't something we
// can know until it's actually configured and tested.
